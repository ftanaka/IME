...
function [ind, x, d, I, invB] = fase2(A, b, c, m, n, x, I, invB)
    ...
    %
    % Calcula solução ótima para o PL: min c'x, S.A.: Ax = b, x >= 0, sabendo
...
function [redc, u, ij] = custoDirecao(A, invB, c, n, m, I)
    ...
    %   I: estrutura de indices básicos (I.b) e não básicos (I.n)
    %
    % Procura por uma direção a partir de uma solução (consegue-se isso, pois
    % temos invB)
    % na qual o custo reduzido (custo na solução encontrada a partir da direção)
    % é menor 
    % que zero.
    %
    % Retorna:
    ...
    %   Caso não haja custo reduzido menor que 0, retorna-se redc = 0, u = [],
    %   ij = -1
    %
    % Esta função é O(nm)
    ...
    while j <= n - m
        ...
        if redc < -1e-10
            ...
            % Retorna o primeiro custo reduzido < 0. (Regra anti-ciclagem pelo
            % menor indice)
            return; 
        end
...
function [imin, teta] = calculaTeta(x, u, I)
    ...
    %   Caso não haja d_b(i) < 0, isto é o custo ótimo é -inf, retorna-se imin =
    %   -1, teta = inf
    %
    % Essa função é O(m)
...
function [I, invB] = atualizaBase(I, invB, u, imin, ij, m)
    ...
    %   m: número de restrições
    % 
    % Recalcula invB e I, de forma que x(I.b(imin)) saia da base e x(I.n(ij))
    % entre.
    % 
    % Retorna:
...
function I = calculaBase(x, n, m);
    ...
    % verifica se o x passado é degenerado ou não. Isso é, se x tem de fato (n -
    % m) zeros
...
function [I, A, invB, m] = removeArtificials(A, I, invB, m, n, b)
    ...
    %   n: número de variáveis
    %
    % Remove da base os índices das variáveis artificiais. Se for necessário,
    % deleta linhas LD de A
    %
    % Retorna:
    %   I, A, invB, m recalculados.
    %
    % Essa função é O(m)
   disp ( '######## REMOVE ARTIFICIAIS' ) 
    for l = (1 : m)(I.b > n) % indexes of I.b with content greater than n
      ...
      I
        ...
        k = 1
      b_cand(k)
        while ((k <= x) && abs(invB(l, :) * A(:, I.n(b_cand(k)))) <= 1e-10)
         %% por que abs? o_O  % só quero saber se é diferente de zero,
         %% mas corro o risco de não dar exatamente zero
            k = k + 1
        end
        
        if k > x
            % Siginifica que B^-1(l, :) * Aj = 0 para todo j (restrição
            % redundante)
            I.b(l) = []
            m--
            A(l, :) = []
            ...
            % vamos trocar a base do indice l (artificial) para j (não
            % artificial)
            I.n(k)
            u = invB * A(:, I.n(k)) 
...
function printDir(u, I, m)
    ...
    %   m: número de restrições
    % 
    % Imprime índices das variáveis básicas e os valores correspondentes das
    % componentes da direção -d
...
function printResto(x, c, I, ij, imin, teta)
    ...
    printf("|  %*.3f\t%*.3f\t%*s\t%*s\n", cxl, (x'*c), tl, teta, inl, ["x"
num2str(I.n(ij))], outl, ["x" num2str(I.b(imin))]);
    printf("|  \n");
end
